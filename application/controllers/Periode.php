...
public function tambah_data() 
{
    ...
    if ($this->form_validation->run() == FALSE) {
       ...
       $periksa = $this->db->get_where('periode',array('buka'=>'Y'));
       $data = array(
...
{
    /*hanya ada satu yang boleh aktiv*/
    $cek_aktiv = $this->db->get_where('periode',array('buka'=>'Y'));
    if($cek_aktiv->num_rows() > 0 ):
     $data= array('buka'=>$this->input->post('buka'));  
     $this->db->update('periode',$data,array('id_periode'=>$this->input->post('id_periode')));
     $this->session->set_flashdata('message', '<div class="alert alert-success fade-in"><i class="fa fa-check"></i>Data akademik berhasil di aktiv kan.</div>');
     redirect(site_url('periode'));
 else:
